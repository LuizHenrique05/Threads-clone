    ...
    return (
        <article className={`flex w-full flex-col rounded-xl ${isComment ? "px-0 xs:px-7" : "bg-dark-2 p-7"}`}>
            <div className="flex items-start justify-between">
                <div className="flex w-full flex-1 flex-row gap-4">
                    <div className="flex flex-col items-center">
                        <Link href={`/profile/${author.id}`} className="relative h-11 w-11">
                            <Image src={author.image} alt="Author profile icon" fill className="cursor-pointer rounded-full" />
                        </Link>
                ...
                <DeleteThread threadId={JSON.stringify(id)} currentUserId={userId} authorId={author.id} parentId={parentId} isComment={isComment} />
                </div>
